...
func (m *Module) Routes() router.Routes {
	return router.Routes{
		router.POST("/customers", m.handler.Create, m.auth),
		router.PUT("/customers/:id", m.handler.Update, m.auth),
		router.GET("/customers", m.handler.List, m.auth),
		router.GET("/customers/:id", m.handler.Get, m.auth),
	}
}
