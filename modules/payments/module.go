...
func (m *Module) Routes() router.Routes {
	return router.Routes{
		router.POST("/payments/confirm", m.handler.Confirm, m.auth),
		router.GET("/payments", m.handler.List, m.auth),
		router.POST("/payments/:id/evidence", m.handler.AddEvidence, m.auth),
	}
}
