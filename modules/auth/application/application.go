...
type Service interface {
	...
	ListUsers(ctx context.Context, page, pageSize int) (common.Page[domain.User], error)
...
func NewUseCase(repo domain.Repository, tokenProvider TokenProvider, oidcProvider OIDCProvider, publisher contracts.EventPublisher) *UseCase {
	return &UseCase{repo: repo, tokenProvider: tokenProvider, oidcProvider: oidcProvider, publisher: publisher}
}
...
func (u *UseCase) Register(ctx context.Context, input RegisterInput) (*domain.User, error) {
	...
	user, err := u.repo.Create(ctx, entity)
	if err != nil {
		return nil, err
	}

	// Publica UserRegistered no barramento de eventos
	if u.publisher != nil {
		evt := contracts.NewUserRegistered(user.ID, user.Email, user.FirstName, user.LastName)
		if pubErr := u.publisher.Publish(ctx, contracts.TopicUserRegistered, evt); pubErr != nil {
			log.Printf("[auth] WARN: falha ao publicar UserRegistered: %v", pubErr)
		}
	}
...
func (u *UseCase) OIDCAuthURL() (infrastructure.OIDCAuthParams, error) {
	if u.oidcProvider == nil {
		return infrastructure.OIDCAuthParams{}, problem.NotImplemented("provedor OIDC não configurado")
	}
...
func (u *UseCase) OIDCExchange(ctx context.Context, input OIDCExchangeInput) (string, error) {
	if u.oidcProvider == nil {
		return "", problem.NotImplemented("provedor OIDC não configurado")
	}
...
func (u *UseCase) ListUsers(ctx context.Context, page, pageSize int) (common.Page[domain.User], error) {
	if page < 1 {
		page = 1
	}
	if pageSize < 1 {
	...
	if err != nil {
		return common.Page[domain.User]{}, err
	}

	items, err := u.repo.List(ctx, page, pageSize)
	if err != nil {
		return common.Page[domain.User]{}, err
	}

	return common.NewPage(items, page, pageSize, total), nil
}
