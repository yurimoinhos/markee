...
func (_c *OrderCreate) SetSagaID(v string) *OrderCreate {
	_c.mutation.SetSagaID(v)
	return _c
}
...
func (_c *OrderCreate) SetAmount(v uint64) *OrderCreate {
...
func (_c *OrderCreate) SetNillableDescription(v *string) *OrderCreate {
	if v != nil {
		_c.SetDescription(*v)
	}
	return _c
...
func (_c *OrderCreate) check() error {
	if _, ok := _c.mutation.SagaID(); !ok {
		return &ValidationError{Name: "saga_id", err: errors.New(`ent: missing required field "Order.saga_id"`)}
	}
	if v, ok := _c.mutation.SagaID(); ok {
...
func (_c *OrderCreate) createSpec() (*Order, *sqlgraph.CreateSpec) {
	...
	if value, ok := _c.mutation.SagaID(); ok {
