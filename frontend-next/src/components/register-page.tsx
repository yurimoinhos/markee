...
export function RegisterPage() {
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      const response = await fetch(`${APP_BASE_PATH}/api/auth/register`, {
        method: 'POST',
        body,
      });
      ...
      if (!response.ok) {
        ...
        throw new Error(extractProblemMessage(payload, 'Falha ao registrar usuário'));
      }
  ...
  return (
    <main className="grid min-h-screen place-items-center px-4 py-10">
      <section className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-6 shadow-xl">
        ...
        <p className="mt-1 text-sm text-slate-600">Cadastre-se para acessar o console.</p>
